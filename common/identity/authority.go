...
func (auth *Authority) MarshalJSON() ([]byte, error) {
	...
	if auth.AuthorityChainID == nil {
		auth.AuthorityChainID = primitives.NewZeroHash()
	}
	if auth.ManagementChainID == nil {
		auth.ManagementChainID = primitives.NewZeroHash()
	}
	if auth.MatryoshkaHash == nil {
