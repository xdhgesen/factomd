...
func (m *RevealEntryMsg) Validate(state interfaces.IState) int {
	...
	if commit == nil {
		state.LogPrintf("executeMsg", "Hold, no commit")
		return 0
	}
	//
	...
	if okEntry {
		...
		if m.Entry.KSize() > 10 {
			state.LogPrintf("executeMsg", "Drop, oversized")
			return -1
		}
		...
		if eb == nil {
			state.LogPrintf("executeMsg", "Hold, no chain")
		...
		if !CheckChainID(state, m.Entry.ExternalIDs(), m) {
			state.LogPrintf("executeMsg", "Drop, chainID does not match hash of ExtIDs")
			return -1
		}
	}
