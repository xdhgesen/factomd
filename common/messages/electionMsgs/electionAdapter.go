...
func (ea *ElectionAdapter) Execute(msg interfaces.IMsg) interfaces.IMsg {
	...
	ea.Election.State.LogMessage("ElectionTrace", "Exec", msg)
	...
	if resp != nil {
		ea.Election.State.LogPrintf("ElectionTrace", "return %d", ea.Electing)
		...
		ea.Election.LogMessage("ElectionTrace", "response", expandedResp.(interfaces.IMsg))
		return expandedResp.(interfaces.IMsg)
	} else {
		ea.Election.State.LogPrintf("ElectionTrace", "return nil")
	}
