...
func NewElectionAdapter(e *elections.Elections, dbHash interfaces.IHash) *ElectionAdapter {
	...
	e.LogPrintf("election", "NewElectionAdapter %d-:-%d for %d (vm %d)", e.DBHeight, e.Minute, e.Electing, e.VMIndex)
	checkAuth.CheckAuthSetsMatch("NewElectionAdapter", e.GetFedServers(), e.GetAuditServers(), e.State.GetFedServers(uint32(e.DBHeight)), e.State.GetAuditServers(uint32(e.DBHeight)), e.State)
...
func (ea *ElectionAdapter) Execute(msg interfaces.IMsg) interfaces.IMsg {
	...
	if simmessage == nil {
		...
		panic("ElectionAdapter.Execute() message is nil")
		//return nil
	}
	...
	if resp != nil {
		ea.Election.LogMessage("election", fmt.Sprintf("for %d (vm %d)", ea.Electing, ea.VMIndex), msg.(interfaces.IMsg))
