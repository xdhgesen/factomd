...
func NetworkProcessorNet(w *worker.Thread, fnode *fnode.FactomNode) {
	Peers(w, fnode)
	w.Run(func() { NetworkOutputs(w, fnode) })
	w.Run(func() { InvalidOutputs(w, fnode) })
}
...
func NetworkOutputs(w *worker.Thread, fnode *fnode.FactomNode) {
	for {
		select {
		case <-w.Exit:
			return
		default:
			_NetworkOutputs(fnode)
		}
	}

}
func _NetworkOutputs(fnode *fnode.FactomNode) {
...
func InvalidOutputs(w *worker.Thread, fnode *fnode.FactomNode) {
	for {
		select {
		case <-w.Exit:
			return
