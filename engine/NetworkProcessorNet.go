...
func NetworkProcessorNet(fnode *fnode.FactomNode) {
	go Peers(fnode)
	go NetworkOutputs(fnode)
	go InvalidOutputs(fnode)
}

func Peers(fnode *fnode.FactomNode) {
...
func sendToExecute(msg interfaces.IMsg, f *fnode.FactomNode, source string) {
	t := msg.Type()
	switch t {
	case constants.MISSING_MSG:
		f.State.LogMessage("mmr_response", fmt.Sprintf("%s, enqueue %d", source, len(f.State.MissingMessageResponseHandler.MissingMsgRequests)), msg)
		f.State.MissingMessageResponseHandler.NotifyPeerMissingMsg(msg)

	case constants.COMMIT_CHAIN_MSG:
		f.State.ChainCommits.Add(msg) // keep last 100 chain commits
		Q1(f, source, msg)            // send it fast track
		reveal := f.State.Reveals.Get(msg.GetHash().Fixed())
		if reveal != nil {
			Q1(f, source, reveal) // if we have it send it fast track
	...
	case constants.REVEAL_ENTRY_MSG:
		...
		if f.State.ChainCommits.Get(msg.GetHash().Fixed()) != nil {
			Q1(f, source, msg) // fast track chain reveals
		} else {
			Q2(f, source, msg) // all other reveals are slow track
			f.State.Reveals.Add(msg)
		}

	case constants.COMMIT_ENTRY_MSG:
		Q2(f, source, msg) // slow track
	...
	default:
		...
		Q1(f, source, msg) // fast track
	}

	if constants.NeedsAck(msg.Type()) {
		// send msg to MMRequest processing to suppress requests for messages we already have
		f.State.RecentMessage.NewMsgs <- msg
	}
}

func Q1(f *fnode.FactomNode, source string, msg interfaces.IMsg) {
	f.State.LogMessage("NetworkInputs", source+", enqueue", msg)
	f.State.LogMessage("InMsgQueue", source+", enqueue", msg)
	f.State.InMsgQueue().Enqueue(msg)
}

func Q2(f *fnode.FactomNode, source string, msg interfaces.IMsg) {
	f.State.LogMessage("NetworkInputs", source+", enqueue2", msg)
	f.State.LogMessage("InMsgQueue2", source+", enqueue2", msg)
	f.State.InMsgQueue2().Enqueue(msg)
}

func NetworkOutputs(f *fnode.FactomNode) {
	for {
		// if len(f.State.NetworkOutMsgQueue()) > 500 {
		// 	fmt.Print(f.State.GetFactomNodeName(), "-", len(f.State.NetworkOutMsgQueue()), " ")
		// }
		//msg := <-f.State.NetworkOutMsgQueue()
		msg := f.State.NetworkOutMsgQueue().BlockingDequeue()

		NetworkOutTotalDequeue.Inc()
		f.State.LogMessage("NetworkOutputs", "Dequeue", msg)
		...
		if msg.IsLocal() {
			...
			f.State.LogMessage("NetworkOutputs", "Drop, local", msg)
			continue
		}
		if msg.GetRepeatHash() == nil {
			f.State.LogMessage("NetworkOutputs", "Drop, no repeat hash", msg)
			continue
		}

		regex, _ := f.State.GetOutputRegEx()
		if regex != nil {
			t := ""
			if mm, ok := msg.(*messages.MissingMsgResponse); ok {
				t = fmt.Sprintf("%7d-:-%d %s", f.State.LLeaderHeight, f.State.CurrentMinute, mm.MsgResponse.String())
			} else {
				t = fmt.Sprintf("%7d-:-%d %s", f.State.LLeaderHeight, f.State.CurrentMinute, msg.String())
			}

			if mm, ok := msg.(*messages.MissingMsgResponse); ok {
				if eom, ok := mm.MsgResponse.(*messages.EOM); ok {
					t2 := fmt.Sprintf("%7d-:-%d %s", f.State.LLeaderHeight, f.State.CurrentMinute, eom.String())
					messageResult := regex.MatchString(t2)
					if messageResult {
						f.State.LogMessage("NetworkOutputs", "Drop, matched filter Regex", msg)
						continue
					}
				}
			}
		...
		//	f.State.Replay.IsTSValidAndUpdateState(
		//		constants.NETWORK_REPLAY,
		//		msg.GetRepeatHash().Fixed(),
		//		msg.GetTimestamp(),
		//		f.State.GetTimestamp())
		//}
		...
		if msg.IsPeer2Peer() {
			...
			if len(f.Peers) > 0 {
				if p < 0 {
					f.P2PIndex = (f.P2PIndex + 1) % len(f.Peers)
					p = rand.Int() % len(f.Peers)
				}
				peer := f.Peers[p]
				if !f.State.GetNetStateOff() { // don't Send p2p messages if he is OFF
					// Don't do a rand int if drop rate is 0
					if f.State.GetDropRate() > 0 && rand.Int()%1000 < f.State.GetDropRate() {
						//drop the message, rather than processing it normally

						f.State.LogMessage("NetworkOutputs", "Drop, simCtrl", msg)
					} else {
						preSendTime := time.Now()
						f.State.LogMessage("NetworkOutputs", "Send P2P "+peer.GetNameTo(), msg)
						peer.Send(msg)
						sendTime := time.Since(preSendTime)
						TotalSendTime.Add(float64(sendTime.Nanoseconds()))
						if f.State.MessageTally {
							f.State.TallySent(int(msg.Type()))
						}
					}
				} else {

					f.State.LogMessage("NetworkOutputs", "Drop, simCtrl X", msg)
				}
			} else {
				f.State.LogMessage("NetworkOutputs", "Drop, no peers", msg)
			}
		} else {
			f.State.LogMessage("NetworkOutputs", "Send broadcast", msg)
			for i, peer := range f.Peers {
				...
				if i != p || wt > 1 {
					...
					if !f.State.GetNetStateOff() { // Don't send him broadcast message if he is off
						if f.State.GetDropRate() > 0 && rand.Int()%1000 < f.State.GetDropRate() && !msg.IsFullBroadcast() {
							//drop the message, rather than processing it normally
...
func InvalidOutputs(f *fnode.FactomNode) {
	for {
		time.Sleep(1 * time.Millisecond)
		_ = <-f.State.NetworkInvalidMsgQueue()
