...
echo "EOMs " $pattern
grep -E $pattern ${fnode:-fnode0}_networkinputs.txt ${fnode:-fnode0}_executemsg.txt ${fnode:-fnode0}_process.txt ${fnode:-fnode0}__processlist.txt | grep -v Drop | grep -iE "enq|xecu|retry|process.txt.*done|add" | ./merge.sh | less
