#!/usr/bin/env bash
if [ "$1" == "help"  ]; then
    echo "... | minutelength.sh <column> <format{sec or hms}> "
echo "Check minute lengths"    
echo 'grep "done.*EOM.*./0/" fnode0_processlist.txt  | ./minutelength.sh'

echo "Check block lengths"    
echo 'grep -E "done.*EOM-DBh/VMh/h [0-9]+/0/-- minute 0" fnode0_process.txt | minutelength.sh'
