...
func TestSetupANetwork(t *testing.T) {
	...
	for i := 0; i < expectedLeaderCount; i++ {
...
func TestBasicSetup(t *testing.T) {
	...
	/*
		WaitMinutes(state0, 2)
		runCmd("F100") // Add 100ms of network delay
		runCmd("S10") // drop 1% (2.1 digit fixed point)
		runCmd("g10")
		...
		WaitBlocks(state0, 1)
		runCmd("9")
		runCmd("x") // bring him back online
		runCmd("8")
		...
		runCmd("i") // some identity command .. unclear why it's here, probably just for coverage
		// Test block recording lengths and error checking for pprof
		runCmd("b100") // cause debug command errors for coverage
		runCmd("b") // cause debug command errors for coverage
		runCmd("babc") // cause debug command errors for coverage
		runCmd("b1000000") // cause debug command errors for coverage

		runCmd("/") // change sorting order
		runCmd("/") // change sorting order back

		runCmd("a1") // exercise a bunch of dump command for code coverage
		runCmd("e1")
		runCmd("d1")
		runCmd("f1")
		runCmd("a100")
		...
		runCmd("yh")
		runCmd("yc")
		runCmd("r")   // walk API around the nodes to exercise all the nodes
		WaitMinutes(state0, 1)
		runCmd("g1")
		runCmd("2")
		runCmd("x")
