...
func (ss *SaveState) RestoreFactomdState(s *State) { //, d *DBState) {
	...
	for coin := range ss.FactoidBalancesP {
		for k := range ss.FactoidBalancesP[coin] {
			s.FactoidBalancesP[coin][k] = ss.FactoidBalancesP[coin][k]
			s.LogPrintf("factoids", "%x<%s> [%s]= %d",
				k,
				primitives.ConvertFctAddressToUserStr(factoid.NewAddress(k[:])),
				constants.TokenNames[coin],
				s.FactoidBalancesP[coin][k])
		}
	}
