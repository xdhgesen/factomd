...
func (s *State) makeMMRs(asks <-chan askRef, adds <-chan plRef, dbheights <-chan int) {
	...
	addAsk := func(ask askRef) {
		...
		ack, msg := s.RecientMessage.GetAckAndMsg(ask.DBH, ask.VM, ask.H, s)
	...
	for {
		...
		select {
		case msg := <-s.RecientMessage.NewMsgs:
			s.LogPrintf("mmr", "start msg handling")
			s.RecientMessage.Add(msg) // adds messages to a message map for MMR
