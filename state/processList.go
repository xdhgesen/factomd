		...
		for j := vm.Height; j < len(vm.List); j++ {
			...
			if (vm.LeaderMinute < 2 && diff <= 3) || diff <= 2 {
				...
				if msg.Process(p.DBHeight, state) { // Try and Process this entry
					...
					p.State.LogMessage("process", fmt.Sprintf("done %v/%v/%v", dbht, i, j), msg)
					...
					p.State.LogMessage("process", fmt.Sprintf("retry %v/%v/%v", dbht, i, j), msg)
...
func (p *ProcessList) AddToProcessList(ack *messages.Ack, m interfaces.IMsg) {
	...
	p.adds <- plRef{ack.DBHeight, ack.VMIndex, int(ack.Height)}
}
...
func NewProcessList(state interfaces.IState, previous *ProcessList, dbheight uint32) *ProcessList {
	...
	// allocate the FUTURE directory block, not the current or previous...
