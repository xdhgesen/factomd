...
func (list *DBStateList) ProcessBlocks(d *DBState) (progress bool) {
	...
	// Any balance that changed in the temp balance for a coin, we want to move that balance in
	for coin := 0; coin < constants.NumberOfTokens; coin++ {
		for k := range pl.FactoidBalancesT[coin] {
			list.State.FactoidBalancesPapi[coin][k] = list.State.FactoidBalancesP[coin][k]
		}
	}
...
func (list *DBStateList) SaveDBStateToDB(d *DBState) (progress bool) {
	...
	s.FactoidBalancesPapi = s.FactoidBalancesPapi[:0]
	for i := 0; i < constants.NumberOfTokens; i++ {
		s.FactoidBalancesPapi = append(s.FactoidBalancesPapi, map[[32]byte]int64{})
	}
	for k := range pl.FactoidBalancesT {
