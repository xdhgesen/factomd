...
func GetHardCodedGrants() []HardGrant {
	...
	case "LOCAL":
		...
		hardcodegrants = []HardGrant{ // KLUDGE: try to add grants for a test address
			HardGrant{11, 1, validateAddress(testAddress)},
			HardGrant{21, 2, validateAddress(testAddress)},
			HardGrant{31, 3, validateAddress(testAddress)},
			HardGrant{41, 4, validateAddress(testAddress)},
