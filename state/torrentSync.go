...
func (s *State) StartTorrentSyncing() error {
	...
	for {
		...
		if s.IsLeader() || s.TorrentUploader() {
			...
			if done < s.GetEntryDBHeightComplete() {
				for done < s.GetEntryDBHeightComplete() {
					s.UploadDBState(done)
					done++
				}
			} else {
				// If we did not just launch, and we are synced, and uploaded --> Long sleep
				if s.GetEntryDBHeightComplete() > 0 && s.GetHighestKnownBlock() == s.GetEntryDBHeightComplete() {
		...
		if upper-(BATCH_SIZE*2) < lower {
			lower = s.GetEntryDBHeightComplete() + 1
	...
	RequestLoop:
		...
		if lower > s.EntryBlockDBHeightComplete {
			s.DBStateManager.RetrieveDBStateByHeight(s.GetEntryDBHeightComplete() + 1)
		}
		// This tells our plugin to ignore any heights below this for retrieval
		s.DBStateManager.CompletedHeightTo(s.GetEntryDBHeightComplete())
