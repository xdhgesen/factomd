...
func (s *State) GotHeartbeat(heartbeatTS interfaces.Timestamp, dbheight uint32) {
	...
	//	s.LogPrintf("executeMsg", "GotHeartbeat(%s, dbht=%d)", heartbeatTS.String(), dbheight)
...
func (s *State) SetLeaderTimestamp(ts interfaces.Timestamp) {
	s.LogPrintf("executeMsg", "SetLeaderTimestamp(%s [%d] %dms) @ %s", ts.String(), ts, ts.GetTimeMilli(), atomic.WhereAmIString(1))
