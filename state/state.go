...
func (s *State) Init() {
	...
	s.InvalidMessages = make(map[[32]byte]interfaces.IMsg, 0)      // Map of invalid messages
	...
	s.apiQueue = NewAPIQueue(10000)                                //incoming message queue from the API
	...
	s.eomQueue = make(chan interfaces.IMsg, 100)                   //queue of EOM (because we sync on these)
