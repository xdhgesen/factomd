...
func (s *State) ReviewHolding() {
	...
	if now.GetTimeMilli()-s.ResendHolding.GetTimeMilli() < 50 {
		return
	}
...
func (s *State) ProcessEOM(dbheight uint32, msg interfaces.IMsg) bool {
	...
	if s.EOMDone {
		...
		if s.EOMProcessed <= 0 { // why less than or equal?
			...
			s.TimeOffset = (s.SyncEnd.Sub(s.SyncTick)) / 8
			...
			//output := KP*float64(SError) + KI*float64(s.SyncIntegral) + KD*float64(derivative) + float64(time.Duration(2*time.Second))
			//s.SyncError = SError
			//
			//s.TimeOffset = -time.Duration(output)
			//if s.TimeOffset > time.Duration(60*time.Second) {
			//	s.LogPrintf("eomsync", "Durration too long %s", s.TimeOffset.String())
			//	s.TimeOffset = 60 * time.Second
			//}
			//if s.TimeOffset < time.Duration(-60*time.Second) {
			//	s.LogPrintf("eomsync", "Durration too low %s", s.TimeOffset.String())
			//	s.TimeOffset = time.Millisecond
			//}
